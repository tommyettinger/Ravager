(ns gaunt.ravager.core
  (:use gaunt.ravager.logic) ; primitive-math)
...
(declare dd shown nucky highlight-front highlight-back
         monster-tiles monster-tile-sizes health-pc health-mob health-red)
(declare clean-bones)
(def dun (atom nil))
...
(defn make-entity
  ([tex hue ^long pos ^long d]
    (assoc (u/create-entity tex)
         :x (double (+ (* 16 (rem pos wide)) (* 8 (- wide (quot pos wide)))))
         :y (- 64.0 (double (* 16 (quot pos wide)))) ;(double (game :height))
         :d d
         :width 48.0
         :height 64.0
         :pos pos
...
(defn make-highlights
  [back front & positions]
  (let[anim0 (double (rem (quot (System/currentTimeMillis) 30) 64))
                 anim  (if (>= anim0 32)
                         (+ 32 (* -1 (- anim0 32)))
                         anim0)
                 r-anim0 (double (/ (rem       (quot (System/currentTimeMillis) 10) 50) 100))
                 g-anim0 (double (/ (rem (+ 10 (quot (System/currentTimeMillis) 10)) 50) 100))
                 b-anim0 (double (/ (rem (+ 20 (quot (System/currentTimeMillis) 10)) 50) 100))
                 r-anim (- 1.0 (* 2 (if (>= r-anim0 0.25)
                         (+ 0.25 (* -1 (- r-anim0 0.25)))
                         r-anim0)))
                 g-anim (- 1.0 (* 2 (if (>= g-anim0 0.25)
                         (+ 0.25 (* -1 (- g-anim0 0.25)))
                         g-anim0)))
                 b-anim (- 1.0 (* 2 (if (>= b-anim0 0.25)
                         (+ 0.25 (* -1 (- b-anim0 0.25)))
                         b-anim0)))]
    (concat
    (mapv #(assoc (u/create-entity back)
         :x (double (+ (* 16 (rem % wide)) (* 8 (- wide (quot % wide)))))
         :y (double (- 64 (- (* 16 (quot % wide))
               anim)))
         :d 1
         :width 48.0
         :height 64.0
         :pos %
         :hue (color r-anim g-anim b-anim 1))
          positions)
    (mapv #(assoc (u/create-entity front)
         :x (double (+ (* 16 (rem % wide)) (* 8 (- wide (quot % wide)))))
         :y (double (- 64 (- (* 16 (quot % wide))
               anim)))
         :d 3
...
(defn make-player
  ([tex ^long pos sizes]
    [(assoc (u/create-entity tex)
         :x (double (+ (* 16 (rem pos wide)) (* 8 (- wide (quot pos wide)))))
         :y (- 64.0 (double (* 16 (quot pos wide))))
         :d 2
         :width 48.0
         :height 64.0
         :pos pos
...
(defn sort-entities [ents]
  (sort #(cond
           ...
           (= (quot (long (:pos %1)) wide) (quot (long (:pos %2)) wide)) (cond
                                 (> (long (:d %1)) (long (:d %2))) 1
                                 (= (long (:d %1)) (long (:d %2)))
                                   (if (> (double (:x %1)) (double (:x %2))) 1
                                     (if (and (= (double (:x %1)) (double (:x %2))) (:clip %1)) 1 -1))
                                 :else -1)
           :else 1)
        ents))
...
(defn center-camera
  ([screen]
    (position! screen
      (float (+ (* 16 (rem (long (:pos @player)) wide)) (* 8 (- wide (quot (long (:pos @player)) wide)))))
      (float (- 64.0 (double (* 16 (quot (long (:pos @player)) wide)))))) ;(double (game :height))
    ;(.apply (u/get-obj s :camera) (Gdx/gl10))
    )
  ([screen _]
    (position! screen
      (float (+ (* 16 (rem (long @cursor) wide)) (* 8 (- wide (quot (long @cursor) wide)))))
      (float (- 64.0 (double (* 16 (quot (long @cursor) wide)))))) ; (double (game :height))
    ;(.apply (u/get-obj s :camera) (Gdx/gl10))
    ))
(defn update-screen!
  [screen entities]
...
(defn process-key [keycode screen entities]
  (let [pp (long (:pos @player))]
    (if (= @mode :act)
      (condp = keycode
        (key-code :up)    (do (shift-player player dun (- pp wide2)) (center-camera screen))
        (key-code :down)  (do (shift-player player dun (+ pp wide2)) (center-camera screen))
        (key-code :left)  (do (shift-player player dun (- pp 2)) (center-camera screen))
        (key-code :right) (do (shift-player player dun (+ pp 2)) (center-camera screen))
...
(defscreen main-screen
  :on-show
  (fn [screen entities]
    (let [packed (TextureAtlas. (.internal ^Files Gdx/files "packed-gaunt/pack.atlas"))
          ...
          _ (def highlight-back  (grab "system-tile-back"))
          _ (def highlight-front (grab "system-tile-front"))
          sv (grab "cmap-wall-vertical")
          sh (grab "cmap-wall-horizontal")
          gr (grab "cmap-floor of a room")
          da (grab "cmap-dark part of a room")
          tv (grab "thin-wall-vertical")
          th (grab "thin-wall-horizontal")
          tc (grab "thin-corner")
          ...
          staircase-up (grab "cmap-staircase up")
          staircase-down (grab "cmap-staircase down")
          water  (grab "cmap-water") ; ~
          ice    (grab "cmap-ice") ; ^
          stones (grab "object-venoms-splash of venom-blinding venom") ; &
          rock   (grab "object-rocks-rock");"object-large stones-statue") ; $
                                  ...
                                  (if (= t wall)
                                    ...
                                    (do 
                                      ...
                                      (if (and (= (rem x 2) 1)
                                        ...
                                        (conj! ents
                                          (make-entity (condp = t
                                            10001.0 staircase-up
                                            10002.0 staircase-down
                                            (condp = (aget ^chars shown i)
                                              \~ water
                                              \^ ice
                                              \$ rock
...
;;         (def dd dd1)
         ...
         (let [ddv (vec dd)]
           (reset! stairs-up (first (keep-indexed #(if (= %2 upstairs) %1) ddv)))
           (reset! stairs-down (first (keep-indexed #(if (= %2 downstairs) %1) ddv))))
...
;;         (def colors (vec (repeatedly (* wide high) #(Color. (float 1.0) (float 1.0) (float 1.0) (float 1.0)))))
      ...
      (sort-entities (concat
                       ...
                       (make-highlights highlight-back highlight-front @stairs-up @stairs-down)
